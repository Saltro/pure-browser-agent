import { Bot, MessageSquare, PanelLeftClose, PanelLeftOpen, Pencil, Plus, Settings, Trash2 } from 'lucide-react';
...
export function SessionSidebar() {
  ...
  const sandboxTooltip = isBooted ? 'Connected' : isBooting ? 'Connecting...' : 'Not connected';
  ...
  return (
    <aside className={isCollapsed ? 'sessionSidebar collapsed' : 'sessionSidebar'}>
      <div className="sessionHeader">
        {!isCollapsed ? (
          <>
            <div className="brand"><Bot size={18} /> Pure Agent</div>
            <div className="sessionHeaderActions">
              <span className={`sandboxDot ${sandboxStatus}`} data-tooltip={sandboxTooltip} />
              <Button variant="ghost" size="icon" className="gearIcon" onClick={() => setSettingsOpen(true)} title="Settings">
                <Settings size={16} />
              </Button>
            </div>
          </>
          ...
          <div className="sessionHeaderCollapsed">
            <Button variant="ghost" size="icon" onClick={() => setCollapsed(!isCollapsed)} title="Expand sessions">
      ...
      {!isCollapsed && (
        <div className="sessionToolbar">
          <Button variant="ghost" size="icon" onClick={() => setCollapsed(!isCollapsed)} title="Collapse sessions">
            <PanelLeftClose size={16} />
          </Button>
          <Button variant="outline" size="sm" onClick={createSession}><Plus size={14} /> New</Button>
        </div>
      )}
          ...
          <div key={session.id} className={session.id === activeSessionId ? 'sessionRow active' : 'sessionRow'}>
            ...
            {!isCollapsed && (
              <span className="sessionActions">
                <Button variant="ghost" size="icon" className="sessionIcon" onClick={() => setRenaming({ id: session.id, title: session.title })} title="Rename"><Pencil size={13} /></Button>
                <Button variant="ghost" size="icon" className="sessionIcon danger" onClick={() => setDeleting({ id: session.id, title: session.title })} title="Delete"><Trash2 size={13} /></Button>
              </span>
            )}
          </div>
        ))}
